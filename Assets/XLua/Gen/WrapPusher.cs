...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<NsLib.ResMgr.BaseResLoaderAsyncType>(translator.PushNsLibResMgrBaseResLoaderAsyncType, translator.Get, translator.UpdateNsLibResMgrBaseResLoaderAsyncType);
        ...
        int NsLibResMgrBaseResLoaderAsyncType_TypeID = -1;
		int NsLibResMgrBaseResLoaderAsyncType_EnumRef = -1;
        
        public void PushNsLibResMgrBaseResLoaderAsyncType(RealStatePtr L, NsLib.ResMgr.BaseResLoaderAsyncType val)
        {
            if (NsLibResMgrBaseResLoaderAsyncType_TypeID == -1)
            {
			    bool is_first;
                NsLibResMgrBaseResLoaderAsyncType_TypeID = getTypeId(L, typeof(NsLib.ResMgr.BaseResLoaderAsyncType), out is_first);
				
				if (NsLibResMgrBaseResLoaderAsyncType_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(NsLib.ResMgr.BaseResLoaderAsyncType));
				    NsLibResMgrBaseResLoaderAsyncType_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, NsLibResMgrBaseResLoaderAsyncType_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, NsLibResMgrBaseResLoaderAsyncType_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for NsLib.ResMgr.BaseResLoaderAsyncType ,value="+val);
            }
			
			LuaAPI.lua_getref(L, NsLibResMgrBaseResLoaderAsyncType_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out NsLib.ResMgr.BaseResLoaderAsyncType val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != NsLibResMgrBaseResLoaderAsyncType_TypeID)
				{
				    throw new Exception("invalid userdata for NsLib.ResMgr.BaseResLoaderAsyncType");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for NsLib.ResMgr.BaseResLoaderAsyncType");
                }
				val = (NsLib.ResMgr.BaseResLoaderAsyncType)e;
                
            }
            else
            {
                val = (NsLib.ResMgr.BaseResLoaderAsyncType)objectCasters.GetCaster(typeof(NsLib.ResMgr.BaseResLoaderAsyncType))(L, index, null);
            }
        }
		
        public void UpdateNsLibResMgrBaseResLoaderAsyncType(RealStatePtr L, int index, NsLib.ResMgr.BaseResLoaderAsyncType val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for NsLib.ResMgr.BaseResLoaderAsyncType ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(NsLib.ResMgr.BaseResLoaderAsyncType[]))
			{
			    NsLib.ResMgr.BaseResLoaderAsyncType[] array = obj as NsLib.ResMgr.BaseResLoaderAsyncType[];
				translator.PushNsLibResMgrBaseResLoaderAsyncType(L, array[index]);
				return true;
			}
            return false;
		}
